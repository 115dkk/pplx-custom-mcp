...
| 나무위키 | VPN/IDC 공지, `[편집]` 마커, 분류·네비게이션 상자, 목차 제거. `/raw/` 직접 요청이 막히면 Agent에는 동일 문서의 `/w/` URL을 사용. 과거판은 같은 `rev`의 영어 프런트엔드로 우회하며 정확히 일치하는 판만 채택 |
...
Worker의 직접 fetch는 `robots.txt`를 요청하거나 적용하지 않습니다. 크롤러가 아니라 사용자가 지목한 URL 하나를 확인하는 fetch 유틸이기 때문입니다. Perplexity `fetch_url`의 제공자 측 robots 판정은 호출자가 비활성화할 수 없으므로, 나무위키 `/raw/`는 동일 문서의 `/w/` URL로 우회합니다. `?rev=N` 과거판은 차단되지 않는 영어 프런트엔드의 정확히 같은 `rev`를 확인하며, 현재판이나 다른 판이 돌아오면 폐기합니다. 실패한 경우에만 한국어 문서 URL로 동일 문서 검증 검색 폴백이 계속됩니다.
